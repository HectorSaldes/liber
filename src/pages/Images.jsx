...
export default function Images() {
				...
				if (batch.latency === 0) {
					...
					if (LocalStorageService.saveData(myLocalStorage, dataImage)) {
						...
						messages(
							"error",
							"Image not uploaded",
							"Your image has not been saved",
						);
				}
			} else messages("error", "Problems with the server", "Try again later");
		} else
			messages("error", "Image not valid", "This kind of format is not valid");
	};
	...
	const image = ({ source: { url } }) => (
		<img style={{ width: "100px" }} alt={url} src={url} />
	);
		...
		<div className='p-4'>
			...
			<div className='text-center'>
				...
				<FileUpload
					...
					cancelLabel='Cancel'
					emptyTemplate={
						<p className='m-0'>You can only upload one image at a time</p>
					}
				/>
				...
				<div className='col'>
					<div className='card'>
						<DataTable
							value={liberImages}
							header={
								<div style={{ color: "var(--surface-900)", fontSize: "20px" }}>
									Table of images saved locally
								</div>
							}
							footer={footer}
